...
export default function CategoryBrands({ heading, category }) {
  ...
  const brands = allBrands.filter(
    (brand) => brand.brandCategory && brand.brandCategory.toLowerCase() === category.toLowerCase(),
  )
  ...
  return (
    <div className={styles.trendbrandWrapper}>
      ...
      {isLoading ? (
        <div className={styles.trendbrandLoading}>Loading brands...</div>
      ) : brands.length === 0 ? (
        <div className={styles.trendbrandMessage}>No brands found in {category} category.</div>
      ) : (
