...
export default function ReelDetailPage() {
  ...
  const handleFollowToggle = async (userId) => {
    ...
    try {
      ...
      const isFollowing = reelsData
        .find((reel) => reel.user?._id === userId)
        ...
        <div className={styleshop.smartphoneContainer}>
          <div className={styles.reelsContainer}>
            {reelsData.map((reel, index) => (
              <div key={reel._id} className={stylesReels.reelContainer}>
                <div className={stylesReels.videoContainer}>
                  {reel.video ? (
                    <>
                      ...
                      {isPaused[index] && (
                        <div
                          className={stylesReels.playIconOverlay}
                          onClick={() => handleVideoClick(index)}
                        >
                          <svg
                            width="80"
                ...
                <div className={stylesReels.actions}>
                  <div className={stylesReels.actionItem}>
                    ...
                    >
                      ...
                      <span className={stylesReels.actionCount}>
                        {reel.likes?.length || 0}
                      </span>
                    </button>
                  </div>
                  ...
                  <div className={stylesReels.profileContainer}>
                    ...
                    <div className={stylesReels.profileInfo}>
                      <Link
                        ...
                        className={stylesReels.profileName}
                      >
                ...
                {reel.product?.title && reel.product?.image && (
                  <>
                    <div className={stylesReels.productRow}>
                      <div className={stylesReels.productCard}>
                        <a
                          href={reel.product?.url || reel.caption}
                          onClick={(e) => {
                            e.preventDefault();
                            const referralCode = reel.caption?.match(
                              /referralCode=([A-Za-z0-9]{6})/
                            )?.[1];
                            if (referralCode) {
                              try {
                        ...
                        >
                          ...
                          <div className={stylesReels.productBrand}>
                            {reel.product.title}
                          </div>
                        </a>
                      </div>
                ...
                {showProductModal && (
                  <div
                    className={stylesReels.productModalOverlay}
                    onClick={() => setShowProductModal(false)}
                  >
                    <div
                      className={stylesReels.productModal}
                      onClick={(e) => e.stopPropagation()}
                    >
                      <div className={stylesReels.modalHeader}>
                        <h3>{reelsData[activeReel]?.product?.title}</h3>
                        <button
                          className={stylesReels.closeButton}
                          onClick={() => setShowProductModal(false)}
                        >
                          ×
                        </button>
                      ...
                      <div className={stylesReels.modalContent}>
                        <img
                          src={
                            reelsData[activeReel]?.product?.image ||
                            "/placeholder.svg"
                          }
                          alt={reelsData[activeReel]?.product?.title}
                        />
                        {reelsData[activeReel]?.product?.url && (
                          <a
                            href={reelsData[activeReel]?.product?.url}
                            target="_blank"
                            rel="noopener noreferrer"
                            className={stylesReels.productLink}
                          >
                            Visit Product Page
                          </a>
                        )}
